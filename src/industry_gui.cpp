	...
	virtual void OnClick(Point pt, int widget, int click_count)
	{
		switch (widget) {
			...
			case WID_ID_DROPDOWN_FILTER: {
				...
				list->push_back(new DropDownListStringItem(STR_BUTTON_FILTER_SELECT_ALL,     -2, false));
				list->push_back(new DropDownListStringItem(STR_BUTTON_FILTER_SELECT_NONE,    -1, false));
				list->push_back(new DropDownListItem(-3, false));

				for (IndustryType indt = 0; indt < NUM_INDUSTRYTYPES; ++indt) {
					const IndustrySpec *inds = GetIndustrySpec(indt);
					if (inds->enabled)
						list->push_back(new DropDownListCheckedItem(inds->name, indt, false, IndustryDirectoryWindow::type_filter[indt]));
				}

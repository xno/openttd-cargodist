...
struct CommandsToolbarWindow : Window {
	...
	CommandsToolbarWindow(const WindowDesc *desc, WindowNumber window_number) : Window()
	{
	...
	virtual void OnClick(Point pt, int widget, int click_count)
	{
		...
		switch (widget) {
			...
			case CTW_HINT:
				...
				break;
			case CTW_LOGIN:
				query_widget = CTQ_LOGIN_NAME;
				ShowQueryString(STR_EMPTY, STR_TOOLBAR_COMMANDS_LOGIN_NAME_QUERY, 25, this, CS_ALPHANUMERAL, QSF_NONE);
				break;
			case CTW_LOGIN_CREDENTIALS:
				query_widget = CTQ_LOGIN_CREDENTIALS_NAME;
			...
			default:
				if(widget >= CTW_NS0 && widget < CTW_NSEND){
					char ip[16];
					if(widget < CTW_NSX4 && widget != CTW_NS8) strecpy(ip, "37.157.196.78", lastof(ip));
	...
	void OnQueryTextFinished(char *str)
	{
		...
		switch (this->query_widget) {
			...
			case CTQ_TOWN_ID:
				...
				break;
			case CTQ_LOGIN_NAME:
				query_widget = CTQ_LOGIN_PASSWORD;
				sprintf(this->login_name, "%s", str);
				ShowQueryString(STR_EMPTY, STR_TOOLBAR_COMMANDS_LOGIN_PASSWORD_QUERY, 25, this, CS_ALPHANUMERAL, QSF_NONE);
				break;
			case CTQ_LOGIN_PASSWORD:
				if (this->login_name) {
					sprintf(msg, "!login %s %s", this->login_name, str);
					NetworkClientSendChat(NETWORK_ACTION_CHAT_CLIENT, DESTTYPE_CLIENT, CLIENT_ID_SERVER, msg);
				}
				break;
			case CTQ_LOGIN_CREDENTIALS_NAME:
				query_widget = CTQ_LOGIN_CREDENTIALS_PW;
				sprintf(this->login_name, "%s", str);
				ShowQueryString(STR_EMPTY, STR_TOOLBAR_COMMANDS_LOGIN_PASSWORD_QUERY, 32, this, CS_ALPHANUMERAL, QSF_NONE);
				break;
			case CTQ_LOGIN_CREDENTIALS_PW:
				if (this->login_name) {
					char buf[64];
					uint8 digest[16];
					Md5 checksum;
					checksum.Append(str, strlen(str));
	...
	NWidget(NWID_HORIZONTAL),
		NWidget(NWID_VERTICAL),
			...
			NWidget(NWID_HORIZONTAL, NC_EQUALSIZE),
				...
				//NWidget(WWT_PUSHTXTBTN, COLOUR_GREY, CTW_SYNCCB),SetMinimalSize(40, 20),SetFill(1, 0), SetDataTip(STR_TOOLBAR_COMMANDS_INFO_CAPTION, STR_TOOLBAR_COMMANDS_INFO_TOOLTIP),
			EndContainer(),
			NWidget(NWID_HORIZONTAL, NC_EQUALSIZE),
...
static const WindowDesc _commands_toolbar_desc(
